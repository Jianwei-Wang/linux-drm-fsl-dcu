...
static int radeon_dp_mst_set_be_cntl(struct radeon_encoder *primary, struct radeon_encoder_mst *mst_enc, enum radeon_hpd_id hpd, bool enable)
{
...
//	.set_property = ra_dp_mst_set_property,
	.destroy = radeon_dp_mst_connector_destroy,
};

static struct drm_connector *radeon_dp_add_mst_connector(struct drm_dp_mst_topology_mgr *mgr, struct drm_dp_mst_port *port, char *pathprop)
{
...
radeon_mst_encoder_dpms(struct drm_encoder *encoder, int mode)
{
	...
	if (!radeon_connector) {
	  //		DRM_ERROR("failed to find connector\n");
		return;
	}
...
radeon_dp_mst_init(struct radeon_connector *radeon_connector)
{
	...
	ret = drm_dp_mst_topology_mgr_init(&radeon_connector->mst_mgr, dev->dev, &radeon_connector->ddc_bus->aux, 8, 6, radeon_connector->base.base.id);
	return ret;
}
