...

{
	...
	*/
	...
	wrptr = udl_set_color_depth(wrptr, 0x00);
	/* set base for 16bpp segment to 0 */
	wrptr = udl_set_base16bpp(wrptr, 0);
	/* set base for 8bpp segment to end of fb */
	wrptr = udl_set_base8bpp(wrptr, 0x35e000);

	wrptr = udl_set_vid_cmds(wrptr, adjusted_mode);
	wrptr = udl_enable_hvsync(wrptr, true);
	wrptr = udl_vidreg_unlock(wrptr);
	DRM_INFO("setup mode info %ld\n", wrptr-buf);
...
static struct drm_crtc_helper_funcs udl_helper_funcs = {
	...
	.mode_set_base = udl_pipe_set_base,
	.mode_set_base_atomic = udl_pipe_set_base_atomic,
